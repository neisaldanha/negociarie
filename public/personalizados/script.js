...
let sortKey = 'user_name'; // Chave padrão para ordenação
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('sort-name').addEventListener('click', () => {
        ...
        loadSalaries(); // Recarrega a tabela com a nova ordenação
    });

    // Adiciona evento de clique para ordenar pelo total
    document.getElementById('sort-total').addEventListener('click', () => {
        sortKey = 'total'; // Altera a chave de ordenação para o total
        toggleSortDirection(); // Alterna a direção da ordenação
        loadSalaries(); // Recarrega a tabela com a nova ordenação
    });
...
async function loadSalaries() {
    ...
    if (response.ok) {
        ...
        if (Array.isArray(salaries)) {
            ...
            const sortedSalaries = sortSalaries(filteredSalaries, sortKey, sortDirection);
...
function sortSalaries(salaries, key, direction) {
    return salaries.sort((a, b) => {
        ...
        // Se a chave for 'total', calcula o valor em vez de comparar strings
        if (key === 'total') {
            aValue = a.hourly_rate * a.hours_consumed;
            bValue = b.hourly_rate * b.hours_consumed;
        }

        // Converte para número caso os valores não sejam números
